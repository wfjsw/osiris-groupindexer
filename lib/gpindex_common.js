...
function setGroupExTag(gid, flag, value) {
    return r.table('groupextag').get(parseInt(gid)).run(db_conn)
        .then((ret) => {
            var upd = {};
            upd[flag] = value;
            if (ret)
                return r.table('groupextag').get(parseInt(gid)).update(upd).run(db_conn);
            else {
                upd['id'] = parseInt(gid);
                return r.table('groupextag').insert(upd).run(db_conn);
            }
        })
}

function queryGroupExTag(gid, flag) {
    if (!db_conn) throw 'databaseNotConnected';
    return r.table('groupextag').get(parseInt(gid)).default({}).pluck(flag).run(db_conn)
        .then((ret) => {
            if (!(ret[flag] === undefined)) {
                return ret[flag];
            } else {
...
function getRecByTag(tag) {
    if (!db_conn) throw 'databaseNotConnected';
    return r.table('groups').getAll(tag, {
            index: 'tag'
        }).run(db_conn)
        .then((cursor) => {
