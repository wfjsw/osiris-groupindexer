...
async function processSpamCheck(msg, bot) {
    ...
    if (nospam_enabled && spam_time != 0 && moment().isSameOrBefore(moment.unix(spam_time))) {
        ...
        try {
            ...
            await bot.sendMessage(msg.chat.id, `#ISSPAM #ENFORCED 已检测到并尝试移除已知群发广告用户，有异议请提交工单复核。\n\n${usermsg}\n\n封禁解除时间：${bannedtime}`, {
                reply_to_message_id: msg.message_id
            })
            _ga.tEvent(user, 'noSpam', 'noSpam.kicked')
        } catch (e) {
            await bot.sendMessage(msg.chat.id, `#ISSPAM 已检测到已知群发广告用户，有异议请提交工单复核。如需自动移除，请将机器人设置为管理员。\n\n${usermsg}\n\n封禁解除时间：${bannedtime}`, {
                reply_to_message_id: msg.message_id
            })
...
async function manuallyEnforceNospam(msg, result, bot) {
    ...
    if (spam_time != 0 && moment().isSameOrBefore(moment.unix(spam_time))) {
        ...
        try {
            ...
            const msgl = await bot.sendMessage(msg.chat.id, `#ISSPAM #ENFORCED 200 OK\n\n${usermsg}\n\n封禁解除时间：${bannedtime}`, {
                reply_to_message_id: msg.message_id
            })
            setTimeout(() => {
            ...
            const msgl = await bot.sendMessage(msg.chat.id, `#ISSPAM 401 Unauthorized\n\n${usermsg}\n\n封禁解除时间：${bannedtime}`, {
                reply_to_message_id: msg.message_id
            })
            setTimeout(() => {
                bot.deleteMessage(msg.chat.id, msgl.message_id)
...
async function forceEnlist(msg, result, bot) {
    try {
        ...
        const user = msg.reply_to_message.from
        if (!user) return
        const userStatus = await bot.getChatMember(admin_id, msg.from.id)
        if ((['left', 'kicked'].indexOf(userStatus) > -1) && temp_admin.indexOf(msg.from.id) == -1) return
