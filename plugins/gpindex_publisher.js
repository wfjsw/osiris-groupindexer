...
function initevents() {
    ...
    context.on('new_private_commit', (groupinfo) => {
        ...
        groupinfo.desc = groupinfo.desc.replace(groupinfo.invite_link, '[scrubbed]')
    ...
    context.on('update_private_data', (groupinfo) => {
        ...
        _e.libs['gpindex_common'].getRecord(groupinfo.id)
            .then((ret) => {
                ...
                var link = 'https://t.me/' + _e.me.username + '?start=getdetail=' + groupinfo.id
                // remove link in desc [security reason]
                ret.desc = ret.desc.replace(ret.invite_link, '[scrubbed]')
                if (groupinfo.type == 'channel') text = util.format(langres['updatePrivateChan'], ret.title, ret.tag, ret.desc, ret.id)
                else text = util.format(langres['updatePrivate'], ret.title, ret.tag, ret.desc, ret.id)
                bot.sendMessage(channel_id, text, {
                    disable_web_page_preview: true,
                    reply_markup: {
                        inline_keyboard: [
